...
import ClassroomProofGallery from '@/components/ui/ClassroomProofGallery'
...
export default function ProjectDetail({ project }: { project: Project }) {
  ...
  return (
    <>
      ...
      <section className="relative w-full" style={{ background: 'var(--bg)', paddingTop: '24px', paddingBottom: '64px' }}>
        ...
        <div className="relative z-10 px-4 sm:px-6 md:px-16 max-w-7xl mx-auto w-full">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.2 }}>
            ...
            <div className="flex flex-wrap items-center gap-6">
              ...
              {project.id === 'classroom-behavior-detection' && (
                <a href="https://colab.research.google.com/github/darshanjoshi05/Portfolio/blob/main/YOLOv8x_SCB05.ipynb"
                  target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-2 text-[0.72rem] tracking-[0.1em] uppercase border border-[#f59e0b]/30 text-[#f59e0b] px-5 py-2.5 hover:border-[#f59e0b]/60 hover:bg-[#f59e0b]/5 transition-all">
      ...
      <div className="relative z-10 px-4 sm:px-6 md:px-16 py-12 max-w-7xl mx-auto space-y-24">
        ...
        {/* ── Real Training Proofs (classroom project only) ── */}
        {project.id === 'classroom-behavior-detection' && (
          <FadeUp>
            <SectionHeader num="03" title="Training Results & Proof" />
            <ClassroomProofGallery />
          </FadeUp>
        )}

        {/* ── Gallery / Output ── */}
        <FadeUp>
          <SectionHeader num={project.id === 'classroom-behavior-detection' ? '04' : '03'} title="Screenshots & Output" />
          <GalleryMock items={project.gallery.map(g => ({ ...g, projectId: project.id }))} />
        </FadeUp>

        {/* ── What I Built ── */}
        <FadeUp>
          <SectionHeader num={project.id === 'classroom-behavior-detection' ? '05' : '04'} title="What I Built" />
        ...
        {/* ── Project Insights ── */}
        <FadeUp>
          <SectionHeader num={project.id === 'classroom-behavior-detection' ? '06' : '05'} title="Project Insights" />
