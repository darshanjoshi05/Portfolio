  ...
  return (
    <>
      ...
      <div className="relative h-screen flex items-center overflow-hidden">
        {/* Re-use the same hero scene from home but smaller */}
        <div className="absolute inset-0 z-[1]">
          <ProjectSceneBg />
        </div>
        <div className="absolute inset-0 z-[2] bg-gradient-to-r from-bg/92 via-bg/60 to-transparent pointer-events-none" />

        <div className="relative z-[3] px-4 sm:px-6 md:px-16 pt-28 max-w-7xl mx-auto w-full">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.2 }}>
            ...
            <h1 className="font-serif text-[clamp(2.8rem,6vw,6rem)] font-bold leading-[0.9] tracking-[-0.03em] mb-6 max-w-2xl">
              <span className="gradient-text">{project.name}</span>
            </h1>

            <p className="text-[0.88rem] leading-[1.85] text-muted max-w-xl mb-8">{project.description}</p>
        ...
        <div className="absolute bottom-6 right-6 z-[3] text-[0.6rem] text-muted/50 tracking-[0.1em] flex items-center gap-1.5">
          <span className="text-accent">⟳</span> Drag to rotate scene
        </div>
      </div>

      {/* ══════ MAIN ══════ */}
      <div className="relative z-10 px-6 md:px-16 py-12 max-w-7xl mx-auto">

        {/* Overview strip */}
        <FadeUp className="mb-20">
          <div className="grid grid-cols-2 md:grid-cols-4 border border-white/[0.06] divide-x divide-white/[0.06]">
            {Object.entries(project.overview).map(([k, v]) => (
              <div key={k} className="bg-surface px-6 py-5">
                <div className="text-[0.58rem] tracking-[0.18em] uppercase text-muted mb-1">{k}</div>
                <div className={`font-sans font-bold text-[0.88rem] ${v.toLowerCase().includes('active') || v.toLowerCase().includes('current') || v.toLowerCase().includes('development') ? 'text-accent' : 'text-[#e8f0fe]'}`}>{v}</div>
              </div>
            ))}
          </div>
        </FadeUp>
        ...
        <FadeUp className="mb-24">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {project.bullets.map((b, i) => (
              <div key={i} className="flex gap-4 border border-white/[0.06] bg-surface p-5 hover:border-accent/20 transition-colors">
        ...
        <FadeUp>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-white/[0.06] pt-12">
            <div>
              {prev && (
                <Link href={`/projects/${prev.id}`} className="group flex flex-col gap-1 border border-white/[0.06] bg-surface p-5 hover:border-accent/25 transition-all">
                  <span className="text-[0.6rem] tracking-[0.15em] uppercase text-muted flex items-center gap-2">← Previous</span>
                  <span className="font-sans font-bold text-sm group-hover:text-accent transition-colors">{prev.name}</span>
                </Link>
              )}
            </div>
            <div className="flex items-center justify-center">
              <Link href="/projects" className="text-[0.7rem] tracking-[0.15em] uppercase text-muted border border-white/[0.06] px-5 py-2 hover:border-accent/40 hover:text-accent transition-all">
                All Projects
              </Link>
            </div>
            <div>
              {next && (
                <Link href={`/projects/${next.id}`} className="group flex flex-col gap-1 border border-white/[0.06] bg-surface p-5 hover:border-accent/25 transition-all text-right">
                  <span className="text-[0.6rem] tracking-[0.15em] uppercase text-muted flex items-center gap-2 justify-end">Next →</span>
                  <span className="font-sans font-bold text-sm group-hover:text-accent transition-colors">{next.name}</span>
                </Link>
              )}
            </div>
          </div>
...
/* Small inline bg scene for hero without importing again */
function ProjectSceneBg() {
  return <ProjectScene />
}
